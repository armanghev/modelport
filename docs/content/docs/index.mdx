...
ModelPort already accepts Anthropic-style client requests, but Anthropic-compatible upstream providers are not implemented yet. Selecting an `anthropic_compatible` upstream currently returns `501`.
</Callout>
