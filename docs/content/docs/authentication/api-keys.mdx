...
ModelPort has two separate authentication layers today:

1. a single bearer token for clients calling the proxy
2. per-provider credentials for upstream providers
